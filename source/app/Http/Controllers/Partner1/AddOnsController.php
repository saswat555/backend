...
class AddOnsController extends Controller
{
    ...
    public function new_add_ons(Request $request)
        ...
        for($i=0;$i<$count_box;$i++)
        {
            ...
            ->insertGetId(['add_on_des'=>$add_more[$i],'add_on_price'=>$price[$i],'child_category_id'=>$child_category_list]);
        }
                    
        // foreach($add_more as $more)
        // {
        //   $ins0ert = DB::table('add_ons_des')
        //   ->insertGetId(['add_on_des'=>$more,'add_on_id'=>$insertAO]);
        // }
        // $idsss[] = $insert;
        // foreach($price as $prices)
        // {
        //   $insert = DB::table('add_ons_price')
        //   ->insert(['add_on_price'=>$prices,'add_on_id'=>$insertAO]);
        // }
    ...
    //     return view('admin.add_ons.testadd_on',compact("services","add","add_ons_des"));
    // }
     ...
     public function addmultiple(Request $request)
       ...
       $add_on_id=$request->add_on_id;
    	...
    	$delete=DB::table('add_ons')->where('add_on_id',$request->$add_on_id)->delete();
    	        DB::table('add_ons_des')->where('add_on_id',$request->$add_on_id)->delete();
