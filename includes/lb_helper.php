...
class LicenseBoxAPI{
	...
	public function verify_license($time_based_check = false, $license = false, $client = false){
		...
		if($time_based_check && $this->verification_period > 0){
			...
			$today = date('d-m-Y');
			if(empty($_SESSION["2805a9f7655efa6"])){
				$_SESSION["2805a9f7655efa6"] = '00-00-0000';
			}
			if($type == 1){
				$type_text = '1 day';
			}elseif($type == 3){
			...
			if(strtotime($today) >= strtotime($_SESSION["2805a9f7655efa6"])){
				$get_data = $this->call_api(
					'POST',
